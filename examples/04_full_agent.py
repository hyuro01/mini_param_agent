...
from mini_param_agent.tools import BashTool, EditTool, ReadTool, WriteTool
from mini_param_agent.tools.mcp_loader import load_mcp_tools_async
from mini_param_agent.tools.note_tool import RecallNoteTool, SessionNoteTool


async def demo_full_agent():
    """Demo: Full-featured agent with all capabilities."""
    print("\n" + "=" * 60)
    print("mini_param_agent - All Features Enabled")
    print("=" * 60)

    # Load configuration
    config_path = Path("mini_param_agent/config/config.yaml")
    if not config_path.exists():
        print("❌ config.yaml not found. Please run:")
        print("   cp mini_param_agent/config/config-example.yaml mini_param_agent/config/config.yaml")
        return
    ...
    with tempfile.TemporaryDirectory() as workspace_dir:
        ...
        system_prompt_path = Path("mini_param_agent/config/system_prompt.md")
...
"""
        ...
        try:
            mcp_tools = await load_mcp_tools_async(config_path="mini_param_agent/config/mcp.json")
            if mcp_tools:
...
async def demo_interactive_mode():
    ...
    print("\n" + "=" * 60)
    print("Interactive mini_param_agent")
    print("=" * 60)
    print("\nThis demo shows multi-turn conversation.")
    print("(In production, use `mini_param_agent` for full interactive mode)")
