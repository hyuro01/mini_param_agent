...
本目录包含 mini_param_agent 已有工具、Agent 接口和机器学习实验的示例。Python 导入路径为兼容性保留为 `mini_param_agent`。
...
**前置要求**:
- 已配置 `mini_param_agent/config/config.yaml` 中的 API key
...
3. 直接使用 `mini_param_agent` 进入交互模式

### 路径 2: 深入理解
1. 阅读并运行所有示例 (01 → 04)
2. 阅读对应的测试用例 (`tests/`)
3. 阅读核心实现代码 (`mini_param_agent/`)
4. 尝试修改示例，实现自己的功能
...
```
**解决**: 确保 `mini_param_agent/config/config.yaml` 中的 API Key 或本地服务占位值与所选端点匹配。
