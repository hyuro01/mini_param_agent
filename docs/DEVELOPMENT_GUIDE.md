...
This guide describes the existing mini_param_agent codebase. The Python package remains `mini_param_agent` for import compatibility.
...
```
mini_param_agent/
├── mini_param_agent/              # Core source code
│   ├── agent.py             # Main agent loop
│   ├── llm/                # Anthropic/OpenAI LLM clients
│   ├── cli.py               # Command-line interface
│   ├── config.py            # Configuration loading
│   ├── tools/               # File, shell, note, MCP, skill, and ML experiment tools
...
When running the agent in interactive mode (`mini_param_agent`), the following commands are available:
...
The optional MCP (Model Context Protocol) configuration includes example integrations. They are not active unless configured and enabled:

#### Memory - Knowledge Graph Memory System

**Function**: Provides long-term memory storage and retrieval based on graph database

**Status**: Disabled in the bundled `mcp-example.json`; enable and configure it explicitly.

**Configuration**: No API key is specified in the example; the external MCP server must be available.

**Capabilities**:
- Store and retrieve information across sessions
- Build knowledge graphs from conversations
- Search stored memory entries through the external MCP server
...
1.  Create a new tool file under `mini_param_agent/tools/`.
...
```python
# mini_param_agent/tools/my_tool.py
from mini_param_agent.tools.base import Tool, ToolResult
...
from mini_param_agent.tools.my_tool import MyTool
