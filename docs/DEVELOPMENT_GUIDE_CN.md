...
本文介绍 mini_param_agent 当前代码。为保持导入兼容，Python 包仍叫 `mini_param_agent`。
...
```
mini_param_agent/
├── mini_param_agent/              # 核心源代码
│   ├── agent.py             # 主 Agent 循环
│   ├── llm/                # Anthropic/OpenAI LLM 客户端
│   ├── cli.py               # 命令行接口
│   ├── config.py            # 配置加载
│   ├── tools/               # 文件、Shell、笔记、MCP、技能和 ML 实验工具
│   └── skills/              # 随包提供的技能
├── tests/                   # 测试代码
├── docs/                    # 文档
├── workspace/               # 工作目录
...
在交互模式 (通过 `mini_param_agent` 启动) 下运行 Agent 时，您可以使用以下命令：
...
项目的可选 MCP (模型上下文协议) 配置提供以下集成示例；需要自行配置和启用：

#### Memory - 知识图谱记忆系统

**功能**：基于图数据库，为 Agent 提供长期记忆的存储与检索能力。

**状态**：随包提供的 `mcp-example.json` 中默认禁用，需要显式启用。

**配置**：示例不要求 API Key，但需要外部 MCP 服务可用。
...
1.  在 `mini_param_agent/tools/` 目录下创建一个新的 Python 文件。
