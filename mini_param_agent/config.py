...
class ToolsConfig(BaseModel):
    ...
    enable_ml_experiment: bool = True
...
class Config(BaseModel):
    ...
    def load(cls) -> "Config":
        ...
        if not config_path.exists():
            raise FileNotFoundError("Configuration file not found. Run scripts/setup-config.sh or place config.yaml in mini_param_agent/config/.")
    ...
    def from_yaml(cls, config_path: str | Path) -> "Config":
        ...
        tools_config = ToolsConfig(
            ...
            enable_ml_experiment=tools_data.get("enable_ml_experiment", True),
    ...
    def get_package_dir() -> Path:
        ...
        Returns:
            Path to the mini_param_agent package directory
        """
    ...
    def find_config_file(cls, filename: str) -> Path | None:
        ...
        1) mini_param_agent/config/{filename} in current directory (development mode)
        2) ~/.mini_param_agent/config/{filename} in user home directory
        3) {package}/mini_param_agent/config/{filename} in package installation directory
        ...
        dev_config = Path.cwd() / "mini_param_agent" / "config" / filename
        if dev_config.exists():
            return dev_config

        # Priority 2: User config directory
        user_config = Path.home() / ".mini_param_agent" / "config" / filename
