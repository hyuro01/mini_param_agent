"""ACP (Agent Client Protocol) bridge for mini_param_agent."""
...
from mini_param_agent.cli import add_workspace_tools, initialize_base_tools
from mini_param_agent.config import Config
from mini_param_agent.llm import LLMClient
from mini_param_agent.retry import RetryConfig as RetryConfigBase
from mini_param_agent.schema import Message
...
class MiniMaxACPAgent:
    ...
    async def initialize(self, params: InitializeRequest) -> InitializeResponse:  # noqa: ARG002
        return InitializeResponse(
            ...
            agentInfo=Implementation(name="mini_param_agent", title="mini_param_agent", version="0.1.0"),
        )
...
async def run_acp_server(config: Config | None = None) -> None:
    """Run mini_param_agent as an ACP-compatible stdio server."""
    ...
    logger.info("mini_param_agent ACP server running")
