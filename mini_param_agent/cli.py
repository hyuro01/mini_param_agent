"""
mini_param_agent - Interactive Runtime

Usage:
    mini_param_agent [--workspace DIR] [--task TASK]

Examples:
    mini_param_agent                              # Use current directory as workspace (interactive mode)
    mini_param_agent --workspace /path/to/dir     # Use specific workspace directory (interactive mode)
    mini_param_agent --task "create a file"       # Execute a task non-interactively
"""
...
from mini_param_agent import LLMClient
from mini_param_agent.agent import Agent
from mini_param_agent.config import Config
from mini_param_agent.schema import LLMProvider
from mini_param_agent.tools.base import Tool
from mini_param_agent.tools.bash_tool import BashKillTool, BashOutputTool, BashTool
from mini_param_agent.tools.file_tools import EditTool, ReadTool, WriteTool
from mini_param_agent.tools.mcp_loader import cleanup_mcp_connections, load_mcp_tools_async, set_mcp_timeout_config
from mini_param_agent.tools.note_tool import SessionNoteTool
from mini_param_agent.tools.ml_experiment_tool import MLExperimentTool
from mini_param_agent.tools.skill_tool import create_skill_tools
from mini_param_agent.utils import calculate_display_width
...
def get_log_directory() -> Path:
    ...
    return Path.home() / ".mini_param_agent" / "log"
...
def print_banner():
    ...
    BOX_WIDTH = 58
    banner_text = f"{Colors.BOLD}🤖 mini_param_agent - Multi-turn Interactive Session{Colors.RESET}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="mini_param_agent - AI assistant with ML experiments and general tools",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mini_param_agent                              # Use current directory as workspace
  mini_param_agent --workspace /path/to/dir     # Use specific workspace directory
  mini_param_agent log                          # Show log directory and recent files
  mini_param_agent log agent_run_xxx.log        # Read a specific log file
        """,
    )
    parser.add_argument(
        "--workspace",
    ...
    parser.add_argument(
        ...
        "-v",
        action="version",
        version="mini_param_agent 0.1.0",
    )
    ...
    if config.tools.enable_skills:
        ...
        try:
            ...
            if skills_path.is_absolute():
                ...
                # 1. Current directory (dev mode: ./skills or ./mini_param_agent/skills)
                # 2. Package directory (installed: site-packages/mini_param_agent/skills)
                search_paths = [
                    skills_path,  # ./skills for backward compatibility
                    Path("mini_param_agent") / skills_path,  # ./mini_param_agent/skills
                    Config.get_package_dir() / skills_path,  # site-packages/mini_param_agent/skills
                ]
...
def add_workspace_tools(tools: List[Tool], config: Config, workspace_dir: Path):
    ...
    if config.tools.enable_ml_experiment:
        tools.append(MLExperimentTool(workspace_dir=str(workspace_dir)))
        print(f"{Colors.GREEN}✅ Loaded ML experiment tool{Colors.RESET}")
...
async def run_agent(workspace_dir: Path, task: str = None):
    ...
    if not config_path.exists():
        ...
        print(f"  {Colors.DIM}1) mini_param_agent/config/config.yaml{Colors.RESET} (development)")
        print(f"  {Colors.DIM}2) ~/.mini_param_agent/config/config.yaml{Colors.RESET} (user)")
        ...
        print(
            f"  {Colors.BRIGHT_GREEN}cp mini_param_agent/config/config-example.yaml mini_param_agent/config/config.yaml{Colors.RESET}"
        )
        print()
        print(f"{Colors.DIM}  This will automatically:{Colors.RESET}")
        print(f"{Colors.DIM}    • Copy the bundled configuration template into the development path{Colors.RESET}")
        print(f"{Colors.DIM}    • Let you edit the API key, model, endpoint, and provider{Colors.RESET}")
        print()
        print(f"{Colors.BRIGHT_YELLOW}📝 Manual Setup:{Colors.RESET}")
        user_config_dir = Path.home() / ".mini_param_agent" / "config"
    ...
    from mini_param_agent.retry import RetryConfig as RetryConfigBase
    ...
    if system_prompt_path and system_prompt_path.exists():
        ...
        system_prompt = "You are mini_param_agent, an intelligent assistant that can help users complete various tasks."
    ...
    history_file = Path.home() / ".mini_param_agent" / ".history"
        ...
        try:
            ...
            if user_input.startswith("/"):
                ...
                if command in ["/exit", "/quit", "/q"]:
                    print(f"\n{Colors.BRIGHT_YELLOW}👋 Goodbye! Thanks for using mini_param_agent{Colors.RESET}\n")
                    print_stats(agent, session_start)
                    break
